...
public class ReflectionRenderTexture : MonoBehaviour
{
	...
	void LateUpdate ()
	{
		...
		if (!SystemInfo.supportsRenderTextures)
			...
			GetComponent<Camera>().enabled = false; // no vertex programs - can't do
	...
	void OnPreCull ()
	{
		...
		{			
			...
			if (m_ReflectUpperSide)
			{
				...
				GetComponent<Camera>().worldToCameraMatrix = m_SourceCamera.worldToCameraMatrix * reflection;
			}
			// else just use the source camera
			else
			{
				GetComponent<Camera>().worldToCameraMatrix = m_SourceCamera.worldToCameraMatrix;
			}
